...
//!
//! Handles Streamable HTTP transport including:
//! - SSE stream establishment for server-initiated messages
//! - JSON-RPC request handling via POST
//! - Reconnection recovery compatibility
//! - Auth discovery fallback responses (non-OAuth server compatibility)
...
/// Application state for HTTP transport
#[derive(Clone)]
...
/// This is the main entry point for POST /mcp requests.
pub async fn handle_json_rpc(
    ...
    if let Some(id) = id {
        match result {
            Ok(result) => create_protocol_json_response(
                StatusCode::OK,
                JsonRpcResponse {
            ...
            Err(error) => create_protocol_json_response(
                StatusCode::OK,
                JsonRpcResponse {
                    jsonrpc: JSONRPC_VERSION.to_string(),
                    id,
...
/// SSE stream handler for server-initiated messages.
/// This is the main entry point for GET /mcp requests.
...
/// Handle auth discovery requests that may come when Claude is in failed state
/// and attempts to authenticate. Since this server doesn't support OAuth,
/// we return a proper error response instead of a 404 with empty body.
pub async fn handle_auth_discovery() -> Response {
    // Return a proper JSON error response instead of empty 404
    // This prevents Claude SDK from showing "SDK auth failed: HTTP 404: Invalid OAuth error response"
    with_protocol_version_header(
        (
            ...
            Json(json!({
                ...
                "error": {
                    "code": -32601,
                    "message": "OAuth authentication is not supported by this server. This is a local tmux MCP server that does not require authentication."
                }
            })),
        )
            .into_response(),
...
fn create_protocol_json_response(status: StatusCode, body: JsonRpcResponse) -> Response {
...
/// Create the HTTP transport router with MCP endpoints
pub fn create_transport_router(command_registry: Arc<CommandRegistry>) -> Router {
    Router::new()
        .route("/mcp", get(handle_sse_stream).post(handle_json_rpc))
        .route("/mcp/", get(handle_sse_stream).post(handle_json_rpc))
        // Auth discovery fallback - handles Claude's auth attempts gracefully
        .route(
            "/mcp/auth",
            get(handle_auth_discovery).post(handle_auth_discovery),
        )
        .route(
            "/mcp/auth/",
            get(handle_auth_discovery).post(handle_auth_discovery),
        ...
        .with_state(TransportState { command_registry })
}
